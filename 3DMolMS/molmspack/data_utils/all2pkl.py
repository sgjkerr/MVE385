...
def process_spectrum_parallel(spectrum, encoder):
	# mol array
	...
	return {'title': spectrum['params']['title'], 'mol': mol_arr, 'spec': spec_arr, 'env': env_arr}

def mgf2pkl_parallel(spectra, encoder): 
	data = []
	with Pool(processes=max(os.cpu_count()-4, 1)) as p:
		results = p.starmap(process_spectrum_parallel, [(spectrum, encoder) for spectrum in spectra])
