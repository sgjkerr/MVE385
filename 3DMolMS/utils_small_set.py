...
def bins_to_spectrum(valuelist, w, intenslist, encoder):
    try:
        ...
        print('Error in bins_to_spectrum')
        return None, None
...
def get_bar_spectra(valuelist, intenslist, encoder):
    try:
        ...
        print('Error in get_bar_spectra')
        return None
    return list(y)

def get_mol_spectra(source_file, file_path, encoder):
    try:
        ...
        print(f"'EXC.DAT' not found in {file_path}")
        data = None
    return data, data_bar, center_of_mass

def get_gross_charge(source_file, file_path, encoder):
    try:
        with open(file_path, 'r') as f:
        ...
        print(f"'detailed.out' not found in {file_path}")
        features = None
    return features

def get_xyz_from_gen_file(data, file_path, encoder):
    if encoder['conf_type'] == 'ornl':
        try:
            ...
            print(f"'geo_end.gen' not found in {file_path}")
            data = None
    return data


def get_mol_attributes_file(data, file_path, encoder):
    try:
        mol = MolFromPDBFile(file_path, sanitize=False, removeHs=False, proximityBonding=True)
        ...
        print(f"'smiles.pdb' not found in {file_path}")
    ...
    try:
        ...
        inchikey = Chem.MolToInchiKey(mol, options='-SNon')
    except InchiReadWriteError as e:
        print(f"An error occurred: {e}")
        return None, None, None, None
    
    return data, smiles_string, inchi, inchikey
