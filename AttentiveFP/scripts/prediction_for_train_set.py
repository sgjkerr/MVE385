...
with open('/home/kerrj/Documents/UV_vis_proj/attentivefp/test_env/Dataset_to_run/AttentiveFP/data/train/train.pkl', 'rb') as fval:
    loaded_array_val = pickle.load(fval)

picked_data =[]
for i in range(0,9701):
    random_int = random.randint(0, loaded_array_val.shape[0])

    absorb_row = np.array(loaded_array_val[random_int][1:-1], dtype = float)
    absorb_val.append(absorb_row)

    encoded_smiles_str = loaded_array_val[random_int][0]
...
if MSE == 0:
    df.to_csv('data_attentiveFP_train_cosine.csv', index=False)
else:
    df.to_csv('data_attentiveFP_train_MSE.csv', index=False)
