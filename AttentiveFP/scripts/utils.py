...
class GraphModel():
    ...
    def fit(
        ...
        best_model = None
        target_train = torch.ones((len(y_train)))

        target_val = torch.ones((len(y_val)))
        ...
        for i in range(epoch // interval):
            ...
            metric_train = metric(torch.tensor(y_train), torch.tensor(y_pred),target_train)
            ...
            metric_val = metric(torch.tensor(y_val), torch.tensor(y_pred),target_val)
